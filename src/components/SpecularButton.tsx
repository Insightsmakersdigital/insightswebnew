...
interface Props extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, "color" | "type"> {
  children?: ReactNode;
  size?: "sm" | "md" | "lg";
  radius?: number;
  tint?: string;
...
export default function SpecularButton({
  ...
  const rootClassName = `${styles.specularButton} ${styles[size]}${className ? ` ${className}` : ""}`;
  const rootStyle = {
    "--sb-radius": `${radius}px`,
    "--sb-tint": tint,
  ...
  const fx = <span ref={fxRef} className={styles.specularButtonFx} aria-hidden="true" />;
  const label = <span className={styles.specularButtonLabel}>{children}</span>;
