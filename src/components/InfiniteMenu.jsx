...
/**
 * @param {{ items?: Array<{ image: string, link?: string, title?: string, role?: string, description?: string }>, scale?: number }} props
 */
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <div className={`face-info ${isMoving ? 'inactive' : 'active'}`}>
            <h2 className="face-title">{activeItem.title}</h2>
            {activeItem.role && <p className="face-role">{activeItem.role}</p>}
          </div>
