...
const DriftWall = ({
  ...
  // Starts as the plain, deterministic item order so the very first render
  // matches exactly between server and client (avoiding a hydration
  // mismatch), then gets Fisher-Yates shuffled once the client mounts.
  // Without this, round-robin assignment (item i -> column i % columns)
  // puts every item back in the exact same column on every load, which
  // reads as a fixed grid instead of a drifting wall.
  ...
  const columnItems = useMemo(() => {
    ...
    shuffledItems.forEach((item, i) => cols[i % columns].push(item));
    return cols.map((col) => (col.length ? col : shuffledItems.slice(0, 1)));
  }, [shuffledItems, columns]);
